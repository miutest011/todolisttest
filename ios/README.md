...
**WKWebView 默认不理会网页的 `confirm()` / `alert()`**：不弹窗，直接当成"用户点了取消"。
症状是**点了没反应**，而且一点错误信息都没有。这个项目里靠 `confirm()` 的地方有六处：
导入数据、删清单、删打卡项目、删某一次打卡记录、删两种标签——外壳不接的话它们全都静悄悄地失效。

接法见 `WebAppViewController.swift` 末尾的 `WKUIDelegate`（顺便把 `alert` 和 `prompt` 也接了，以后网页加了也不怕）。

**加了新的系统级交互（弹窗、选文件、分享）之后，一定要在模拟器里点一遍**——
这类问题网页版完全正常，只在外壳里才犯，而且不报错。

Debug 版还打开了 `isInspectable`，Mac 上的 Safari → 开发 → 模拟器 → 就能连进去看控制台，排查这类问题比猜快得多。
